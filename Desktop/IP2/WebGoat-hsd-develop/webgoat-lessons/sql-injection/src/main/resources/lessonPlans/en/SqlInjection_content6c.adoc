...
If the browser responds with a page not found or something else you know a blind SQL injection probably does not work.
...
So but how do we actually take advantage of this? Above we have only asked the database for trivial questions but you can
...
Be aware that this approach probably does not work if the privileges of the database are set up correctly (meaning the
system tables can not be queried with the user. That is used to connect from the web application to the database).


The other way is called a time-based SQL injection, in this case you will ask the database to wait before returning
